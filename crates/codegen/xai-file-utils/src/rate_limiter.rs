//! Token-bucket rate limiting with sustained-overload detection.
//!
//! This module is the single implementation of the limiter used by both the
//! `monitor` tool (`xai-grok-tools`) and the asset transfer jobs
//! (`assets::jobs`): a [`TokenBucket`] caps how many events may be emitted per
//! refill interval, and a [`SuppressionTracker`] counts what was dropped so a
//! single catch-up notice replaces a burst of suppressed events.
//!
//! The monitor re-exports all four types from its own `rate_limiter` module, so
//! nothing downstream has two implementations to keep in sync.
