...
//!   assistant reasoning is stripped (`Strip` echo), and the delta hook
//!   hoists reasoning out of `content` when a vLLM/SGLang server interleaves
//!   reasoning prose into the content field.
...
mod tests {
    ...
    fn vllm_hoists_content_reasoning_and_clears_details() {
        ...
        assert_eq!(delta.reasoning_content.as_deref(), Some("step 1 deep thought"));
