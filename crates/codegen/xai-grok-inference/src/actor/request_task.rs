...
enum AttemptOutcome {
    /// Stream emitted [`InferenceEvent::Completed`] with a non-empty
    /// response.
    Completed {
        response: Box<ConversationResponse>,
        metrics: InferenceLatencyStats,
    },
    /// Stream emitted [`InferenceEvent::Completed`] but the response
    ...
    /// Stream emitted [`InferenceEvent::Failed`]. The captured raw
    ...
    InitFailed { error: InferenceError },
}
...
pub(crate) async fn run_request_task(
    ...
    let mut client = match InferenceClient::new(config.clone()) {
        Ok(c) => c,
        Err(err) => {
    ...
    let sampling_span = crate::inference_log::request_span(
        &request_id,
        &config.model,
    ...
    loop {
        ...
        match outcome {
            AttemptOutcome::Completed {
                ...
                // `run_one_attempt`.
                let _ = event_tx.send(InferenceEvent::Completed {
            ...
            AttemptOutcome::Empty { context } => {
                ...
                let err = InferenceError::EmptyResponse { context };
            ...
            AttemptOutcome::Failed { error } => {
                ...
                if let InferenceError::DoomLoopDetected { .. } = &error {
        ...
        RetryDecision::Fatal(fatal_err) => {
            ...
            if budget_exhausted {
                ...
                let status_code = match err {
                    InferenceError::Api { status, .. } => Some(status.as_u16()),
                    InferenceError::Http(e) => e.status().map(|s| s.as_u16()),
                    _ => None,
                };
...
/// `event_tx`. Captures the rich `InferenceError` from the underlying
...
/// `Result<T, InferenceError>` items unchanged so the L2 transform sees
/// them and converts them to `InferenceErrorInfo` for events.
...
#[allow(clippy::too_many_arguments)]
async fn drive_l2(
    l2: impl futures_util::Stream<Item = InferenceEvent>,
    request_id: RequestId,
    event_tx: &mpsc::UnboundedSender<InferenceEvent>,
    cancel_token: &CancellationToken,
    captured: ErrorCell,
    ...
    loop {
        tokio::select! {
            ...
            next = l2.next() => match next {
                Some(InferenceEvent::Completed { response, metrics, .. }) => {
                    ...
                    if let Some(policy) = doom_check {
                        ...
                        if !triggers.is_empty() {
                            return AttemptOutcome::Failed {
                                error: InferenceError::DoomLoopDetected {
                                    triggers,
                    ...
                    if response.stop_reason == Some(xai_grok_inference_types::StopReason::Error) {
                ...
                Some(other) => {
                    if matches!(
                        ...
                        InferenceEvent::FirstToken { .. }
                            ...
                            | InferenceEvent::BackendToolCallStarted { .. }
                ...
                None => {
                    ...
                    return AttemptOutcome::Failed {
                        error: InferenceError::EventStreamError(
...
fn retag(event: InferenceEvent, _request_id: &RequestId) -> InferenceEvent {
    event
}

/// Reconstruct a [`InferenceError`] from a [`InferenceErrorInfo`] when
...
/// error in the cell.
fn synthesize_from_info(info: &InferenceErrorInfo) -> InferenceError {
    match info.kind {
        InferenceErrorKind::IdleTimeout => InferenceError::IdleTimeout {
            elapsed_secs: info
                .message
                .split_whitespace()
        ...
        InferenceErrorKind::Serialization => {
            InferenceError::serialization_from_rendered(&info.message)
        }
        InferenceErrorKind::Http => InferenceError::EventStreamError(info.message.clone()),
        InferenceErrorKind::Api | InferenceErrorKind::RateLimited => {
            let status = info
                .status_code
        ...
        InferenceErrorKind::MaxTokensTruncation => InferenceError::MaxTokensTruncation,
        InferenceErrorKind::DoomLoopDetected => InferenceError::DoomLoopDetected {
...
fn build_empty_context(
    reason: xai_grok_inference_types::EmptyReason,
...
fn emit_retrying(
    ...
    let info = InferenceErrorInfo::from(err);
    let _ = event_tx.send(InferenceEvent::Retrying {
        request_id: request_id.clone(),
        attempt,
        max_retries,
        kind: info.kind,
...
mod tests {
    ...
    fn synthesize_idle_timeout_extracts_elapsed_secs() {
        let info = InferenceErrorInfo {
            kind: InferenceErrorKind::IdleTimeout,
            status_code: None,
        ...
        match err {
            InferenceError::IdleTimeout { elapsed_secs } => assert_eq!(elapsed_secs, 240),
    ...
    fn synthesize_serialization_stays_serialization() {
        ...
        let original = InferenceError::Serialization(
            serde_json::from_str::<i32>("missing field `delta`").unwrap_err(),
        );
        let info = InferenceErrorInfo::from(&original);
        let err = synthesize_from_info(&info);
        assert!(
            matches!(err, InferenceError::Serialization(_)),
    ...
    async fn tee_captures_first_error_only() {
        let items: Vec<InferenceResult<u32>> = vec![
            Ok(1),
            Err(InferenceError::EventStreamError("first".into())),
            Err(InferenceError::EventStreamError("second".into())),
        ];
