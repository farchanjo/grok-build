...
//! it" buckets.
//!
//! `xai-grok-inference` is intentionally decoupled from `xai-grok-shell`
...
//! into [`crate::InferenceConfig::attribution_callback`]; the sampler
...
//! and return the same `InferenceError::Auth` they would otherwise.
...
/// `InferenceClient` (chat completions, responses, messages -- each in
...
/// own HTTP clients that do not flow through `InferenceClient`, and
...
//
// The `Debug` bound is a structural requirement: [`crate::InferenceConfig`]
...
// `derive(Debug)` on `InferenceConfig`.
pub trait Auth401AttributionCallback: Send + Sync + std::fmt::Debug {
    ...
    /// bearer never leaves [`crate::InferenceClient`]. This is the
