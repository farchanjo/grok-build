...
async fn empty_stream_yields_started_then_completed() {
    let raw = stream::iter(Vec::<Result<MessageStreamEvent, InferenceError>>::new()).boxed();
    ...
    assert!(matches!(events[1], InferenceEvent::Completed { .. }));
}
...
async fn model_context_window_exceeded_fails_as_max_tokens_truncation() {
    ...
    match evs.last().unwrap() {
        InferenceEvent::Failed { error, .. } => {
            assert_eq!(
                error.kind,
                crate::events::InferenceErrorKind::MaxTokensTruncation
            );
...
async fn model_metadata_yielded_after_stream_started() {
    ...
    assert!(matches!(evs[0], InferenceEvent::StreamStarted { .. }));
    assert!(matches!(evs[1], InferenceEvent::ModelMetadata { .. }));
}
