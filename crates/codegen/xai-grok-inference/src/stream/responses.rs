                ...
                ResponseStreamEvent::ResponseFailed(failed_event) => {
                    ...
                    let err = InferenceError::Api {
                        ...
                        error_code: response
                            .error
                            .as_ref()
                            .map(|e| xai_grok_inference_types::ApiErrorCode::parse(&e.code)),
                    };
                    yield InferenceEvent::Failed {
                        request_id: request_id.clone(),
                        error: InferenceErrorInfo::from(&err),
                ...
                ResponseStreamEvent::ResponseError(error_event) => {
                    ...
                    let err = InferenceError::Api {
                        ...
                        error_code: error_event
                            .code
                            .as_deref()
                            .map(xai_grok_inference_types::ApiErrorCode::parse),
                    };
                    yield InferenceEvent::Failed {
                        request_id: request_id.clone(),
                        error: InferenceErrorInfo::from(&err),
...
mod tests {
    ...
    async fn response_failed_yields_failed_500() {
        ...
        match events.last().unwrap() {
            InferenceEvent::Failed { error, .. } => {
                ...
                assert_eq!(
                    error.error_code,
                    Some(xai_grok_inference_types::ApiErrorCode::Other(
                        "server_error".into()
                    ))
                );
            }
    ...
    async fn response_error_event_carries_code_into_failed() {
        let error_event = rs::ResponseStreamEvent::ResponseError(rs_types::ResponseErrorEvent {
            sequence_number: 0,
            code: Some(xai_grok_inference_types::INVALID_IMAGE_ERROR_CODE.into()),
