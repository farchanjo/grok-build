...
impl MemoryStorage {
    ...
    fn new_inner(cwd: &Path, root_override: Option<&Path>, use_workspace_hash: bool) -> Self {
        ...
        let workspace_dir = if use_workspace_hash {
            let workspace_hash = crate::workspace_identity::workspace_storage_identity(cwd);
...
pub use crate::workspace_identity::slugify;

#[cfg(test)]
fn compute_workspace_hash(cwd: &Path) -> String {
    crate::workspace_identity::workspace_storage_identity(cwd)
}

#[cfg(test)]
pub(crate) use crate::workspace_identity::extract_repo_identity;

#[cfg(test)]
fn normalize_remote_url(url: &str) -> Option<String> {
    crate::workspace_identity::normalize_remote_url(url)
}
