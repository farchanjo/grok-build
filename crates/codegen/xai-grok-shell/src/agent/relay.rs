...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!(
            "Authentication required. Connect xAI in /providers (or run `grok provider connect xai`)."
        );
        cancel.cancel();
        return false;
    };
    ...
    match recovered {
        Ok(new_auth) if new_auth.key == config.auth.key => {
            ...
            xai_grok_telemetry::unified_log::info(
                ...
                Some(serde_json::json!({
                    "context": context,
                    "key_prefix": xai_grok_auth::bearer_tail(&new_auth.key),
                })),
            );
            false
        }
        Ok(new_auth) => {
            ...
            xai_grok_telemetry::unified_log::info(
                ...
                Some(serde_json::json!({
                    "context": context,
                    "new_key_prefix": xai_grok_auth::bearer_tail(&new_auth.key),
                })),
            );
