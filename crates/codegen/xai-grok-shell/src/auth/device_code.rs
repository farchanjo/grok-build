...
pub enum DeviceCodeError {
    #[error(
        "Device-code login is not available for this deployment. \
         Connect xAI in /providers (or run `grok provider connect xai`), or set XAI_API_KEY instead."
    )]
    NotEnabled,
    #[error(transparent)]
...
pub async fn complete_device_code_login(
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!(
                "Device code expired. Connect xAI in /providers (or run `grok provider connect xai`) again."
            );
        }
