// Re-export all types from the standalone xai-grok-inference-types crate.
// This keeps all existing `crate::inference::types::*` imports working.
pub use xai_grok_inference_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `xai-grok-inference-types::types` (and
// are re-exported above via the wildcard) so the new
// `xai-grok-inference` crate can reference them without a circular
// dep on `xai-grok-shell`.

// Tests for the types now live in xai-grok-inference-types crate.
...
///
/// Lives in the shell (rather than `xai-grok-inference-types` or
