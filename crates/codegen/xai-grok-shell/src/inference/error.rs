...
//!
//! The canonical error types now live in `xai_grok_inference_types::error`.
...
pub use xai_grok_inference_types::error::*;
...
/// after stripping `API error (status …):` (InferenceError Display prefix).
...
/// Drop `InferenceError::Api`'s Display prefix so users see the IC body, not
...
/// Map a `InferenceError` to an ACP `Error` for client-facing responses.
...
pub fn map_sampling_err_to_acp(err: InferenceError) -> acp::Error {
    ...
    match err {
        ...
        InferenceError::Serialization(_) => acp::Error::invalid_params().data(err.to_string()),
        InferenceError::Api {
            status, message, ..
        } => match status {
        ...
        InferenceError::EventStreamError(message) => acp::Error::internal_error().data(message),
        ...
        InferenceError::IdleTimeout { elapsed_secs } => acp::Error::internal_error().data(format!(
...
pub fn terminal_error_data(
    ...
    if kind != xai_grok_inference::InferenceErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == xai_grok_inference::InferenceErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn format_rate_limited_surfaces_nonempty_server_detail() {
        ...
        // Production detail is InferenceError::Api Display (prefixed).
    ...
    fn rate_limit_code_differs_from_internal_error() {
        let rate_err = InferenceError::Api {
            status: StatusCode::TOO_MANY_REQUESTS,
    ...
    fn stop_reason_for_turn_error_distinguishes_max_tokens() {
        let err = map_sampling_err_to_acp(InferenceError::MaxTokensTruncation);
        assert_eq!(stop_reason_for_turn_error(&err), "MaxTokens");
        assert_eq!(
