...
pub fn assert_route_usable(
    ...
    let lifecycle = load_lifecycle_state(home).unwrap_or_else(|_| ProviderLifecycleState::empty());

    if let Some(prov_inc) = req.provenance_incarnation {
        if let Ok(parsed) = ProviderIncarnation::new(prov_inc) {
            if lifecycle.is_tombstoned(id, &parsed) {
        ...
        // Live descriptor incarnation must match when both present.
    ...
    if let Some(expected_gen) = req.session_registry_generation {
        ...
        if live != 0 && expected_gen != 0 && live != expected_gen {
            // Generation replacement: fail closed for retries and new requests
            // that pinned a prior generation with exact-route provenance.
    ...
    // Validate id grammar so path-like forgeries never pass as sibling borrow.
    if ProviderId::new(id).is_err() {
        return Err(RouteGuardError::ProviderMissing { id: id.to_owned() });
    }

    let _ = req.is_retry; // documented: retries use the same fail-closed path
    Ok(())
}
