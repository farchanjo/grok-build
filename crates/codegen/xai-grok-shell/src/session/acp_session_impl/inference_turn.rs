...
impl SessionActor {
    ...
    fn resolve_hosted(
        ...
        Vec<xai_grok_inference_types::HostedTool>,
        xai_grok_inference_types::ToolOverrides,
    ) {
        let mut tools = self.agent.borrow().hosted_tools().to_vec();
        let applied = xai_grok_inference_types::apply_tool_overrides(
            &mut tools,
    ...
    pub(crate) fn effective_hosted_tools(&self) -> Vec<xai_grok_inference_types::HostedTool> {
        self.resolve_hosted().0
    }
    pub(crate) fn hosted_tools_for_turn(&self) -> Vec<xai_grok_inference_types::HostedTool> {
    ...
    /// inherits it.
    pub(crate) fn set_tool_overrides(&self, overrides: xai_grok_inference_types::ToolOverrides) {
    ...
    pub(crate) fn apply_tool_overrides_update(
        &self,
        update: Option<xai_grok_inference_types::ToolOverridesUpdate>,
    ) {
    ...
    /// Reconstruct a full `InferenceConfig` (with credentials) by combining
    ...
    pub(super) async fn reconstruct_full_config(
        ...
        impl xai_grok_inference::HeaderInjector for TraceContextInjector {
        ...
        let cfg = self
            ...
            .await
            .unwrap_or_else(|| xai_grok_inference_types::InferenceSettings {
                base_url: String::new(),
                model: String::new(),
        ...
        // keep prior behavior. The chat-state `InferenceConfig` may already be
    ...
    /// `xai-grok-inference` instead of constructing a new
    /// `OaiCompatClient`.
    ///
    ...
    async fn handle_sampling_failure_with_context_compaction(
        ...
        if matches!(error.kind, InferenceErrorKind::RateLimited) {
            ...
            let acp_err = acp::Error::new(
                crate::inference::error::RATE_LIMITED_ERROR_CODE,
        ...
        if !matches!(error.kind, InferenceErrorKind::Auth)
        ...
        if matches!(error.kind, InferenceErrorKind::EmptyResponse) {
        ...
        let is_auth_401 =
            error.status_code == Some(401) || matches!(error.kind, InferenceErrorKind::Auth);
    ...
    /// push), then submits via `InferenceHandle::submit_and_collect` and
    /// returns:
    /// * `Ok(InferenceTurnOutcome::Response(_))` - model responded.
    /// * `Ok(InferenceTurnOutcome::CompactAndResubmit)` - compaction
    ...
    pub(crate) async fn run_turn_via_sampler(
            ...
            .await
        {
            Ok((response, metrics)) => {
                ...
                Ok(InferenceTurnOutcome::Response(
                    Box::new(response),
                    Box::new(metrics),
                ))
            }
...
fn resolve_configured_cutoff(
    seed: Option<xai_grok_inference_types::ToolOverrides>,
    base: Option<&xai_grok_inference_types::ToolOverrides>,
) -> xai_grok_inference_types::ToolOverrides {
    use xai_grok_inference_types::{ToolOverrides, WebSearchOptions, XSearchOptions};
    let ToolOverrides {
        x_search: seed_x,
        web_search: seed_w,
...
mod configured_cutoff_tests {
    ...
    fn inherited_cutoff_agrees_with_the_wire_echo() {
        use xai_grok_inference_types::{HostedTool, apply_tool_overrides};
