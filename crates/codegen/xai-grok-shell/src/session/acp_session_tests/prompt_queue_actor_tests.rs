...
#[tokio::test]
async fn manual_compaction_pauses_and_then_resumes_prompt_promotion() {
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (actor, _rx) = build_actor().await;
                ...
                .await;
            {
                ...
                assert!(
                    ...
                    "a manual compaction must not let a queued prompt start a concurrent turn"
                );
                assert_eq!(state.pending_inputs.len(), 1);
            }
            assert!(actor.current_prompt_id.lock().unwrap().is_none());
            ...
            assert!(
                state.running_task.is_some(),
                "promotion must resume once the compaction resolves"
            );
            assert_eq!(state.pending_inputs.front().unwrap().prompt_id, "p1");
            assert_eq!(
                actor.current_prompt_id.lock().unwrap().as_deref(),
