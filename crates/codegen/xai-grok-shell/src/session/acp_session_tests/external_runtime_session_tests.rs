...
#[tokio::test(flavor = "current_thread")]
async fn compact_deeper_entry_refuses_external_without_side_effects() {
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (gateway_tx, _gateway_rx) = tokio::sync::mpsc::unbounded_channel();
            ...
            let error = actor
                .run_compact(None)
                .await
                .expect_err("external compaction must fail closed");
            let data = error.data.as_ref().expect("compaction error data");
            assert!(
                data.to_string().contains("not supported"),
                "unexpected compaction error data: {data}"
            );
            let after = actor.chat_state_handle.snapshot().await.unwrap();
            assert_eq!(after.structural_epoch, before.structural_epoch);
            assert_eq!(after.conversation.len(), before.conversation.len());

            let result = actor
                .execute_builtin_slash_command(slash_commands::BuiltinAction::Compact {
                    user_context: None,
                })
                .await;
            assert!(result.is_err(), "direct slash execution must fail closed");
        })
        .await;
}
