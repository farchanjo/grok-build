...
/// The dynamic policy triggers at the reserve boundary when it is earlier
/// than the fixed percentage threshold.
#[tokio::test(flavor = "current_thread")]
async fn dynamic_auto_compact_uses_output_and_safety_reserves() {
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (gateway_tx, _gateway_rx) =
            ...
            actor
                ...
                .set_compaction_policy(xai_grok_agent::CompactionPolicy {
                    trigger_policy: xai_grok_agent::CompactionTriggerPolicy::Dynamic,
                    ..Default::default()
                });
            let context_window = std::num::NonZeroU64::new(100_000).unwrap();

            assert!(
                actor.should_auto_compact(59_039, context_window).is_none(),
                "dynamic trigger must not fire before the 59,040-token reserve boundary"
            );
            let trigger = actor
                .should_auto_compact(59_040, context_window)
                .expect("dynamic trigger must fire at 100,000 - 32,768 - 8,192");
            assert_eq!(trigger.tokens_used, 59_040);
        })
        .await;
}

/// The dynamic policy never fires later than the configured fixed threshold.
#[tokio::test(flavor = "current_thread")]
async fn dynamic_auto_compact_retains_fixed_threshold_ceiling() {
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (gateway_tx, _gateway_rx) =
            ...
            let context_window = std::num::NonZeroU64::new(1_000_000).unwrap();

            assert!(actor.should_auto_compact(849_999, context_window).is_none());
            assert!(actor.should_auto_compact(850_000, context_window).is_some());
        })
        .await;
}
