...
use common::{create_test_client, create_test_client_with_extra_headers, test_inference_config};
...
async fn test_responses_api_streaming_text() {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        use xai_grok_shell::inference::rs::ResponseStreamEvent;
        match event {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        if let ResponseStreamEvent::ResponseCompleted(completed) = event {
            for output in completed.response.output {
                use xai_grok_shell::inference::rs::OutputItem;
                ...
                match output {
                    ...
                    OutputItem::Message(msg) => {
                        for content in &msg.content {
                            use xai_grok_shell::inference::rs::OutputMessageContent;
...
async fn test_chat_completions_401_unauthorized() {
    ...
    if let Err(InferenceError::Auth(_)) = result {
        // Expected
    } else {
...
/// `InferenceConfig::extra_headers`; this proves the client forwards it on the wire.
#[tokio::test]
...
/// `InferenceConfig::doom_loop_recovery` puts `x-grok-doom-loop-check` on the
...
async fn test_doom_loop_check_enabled_sends_header_and_absorbs_check_event() {
    ...
    let mut config = test_inference_config(&server.url(), ApiBackend::Responses, &[]);
...
async fn test_responses_backend_hits_responses_endpoint_not_chat_completions() {
    ...
    // The session-level dispatch in `xai-grok-inference` selects the
    // backend stream based on `InferenceClient::api_backend()`.
