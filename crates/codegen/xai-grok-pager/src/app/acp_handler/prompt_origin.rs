...
///
/// PR19: `Unknown` (legacy/wire entries without an explicit typed origin) is
/// deliberately NOT server-initiated here — the pager has no typed origin on
/// wire, so a legacy or unrecognized id must render and adopt like a normal
/// user turn (its prime eligibility is gated server-side, not here).
pub(crate) fn is_server_initiated_prompt(prompt_id: &str) -> bool {
    use xai_grok_shell::session::PromptOrigin as PO;
    let origin = PO::from_prompt_id(prompt_id);
    origin.is_synthetic() && !matches!(origin, PO::Unknown)
}
