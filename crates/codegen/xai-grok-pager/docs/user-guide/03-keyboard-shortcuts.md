...
**Esc is not a focus key and never cancels work.** It follows the clear / rewind semantics below; `Ctrl+C` is the only cancel gesture (turns and a running `/compact` alike). Nothing depends on `[ui].vim_mode` (scrollback nav) or `[ui].simple_mode` (prompt editor) for cancellation. Overlays, modals, slash/file dropdowns, voice, search, and selection still steal Esc first.

## Escape

| State | Gesture | Effect |
|--------|---------|--------|
| Turn running (any mode) | `Esc` | Swallowed no-op (does **not** cancel, never arms clear/rewind, preserves the draft). Use `Ctrl+C`. |
| Command in flight (`/compact`) | `Esc` | Swallowed no-op. Use `Ctrl+C` to cancel the compaction. |
| Turn or command cancelling | `Esc` | Re-sends cancel in **every** mode (retry if the first ack was lost). `Ctrl+C` in this state escalates toward quit. |
...
**Post-cancel grace:** for about a second after an Esc-triggered cancel retry, the idle rewind arm stays suppressed — mashing Esc cannot silently open the rewind picker. Only the rewind arm is held; every other Esc behavior is unaffected.

**Steal-Esc (runs before mid-turn swallow and clear / rewind):** overlays, modals, slash/file/completion dropdowns, history search, scrollback search, text selection, link highlight, voice, and **Bash / Remember / Feedback mode exit** when the prompt is empty (Esc leaves `!` / `#` / feedback mode and returns to the normal prompt — even while a turn is running).

**Ctrl+C vs Esc:** with a non-empty draft while a turn is running, Ctrl+C clears the draft and keeps the turn; a second Ctrl+C on an empty prompt cancels. A running `/compact` cancels on the first Ctrl+C (a second press escalates toward quit). Esc never cancels: idle non-empty Ctrl+C clears in one press, Esc requires two presses within 800ms.
