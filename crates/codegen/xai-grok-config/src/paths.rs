...
///
/// On Unix, `DirBuilder` recursive 0700 creates newly made ancestors at
/// 0700; only the requested leaf is re-tightened if it already existed.
...
mod tests {
    ...
    #[test]
    #[cfg(unix)]
    fn ensure_sessions_cwd_dir_retightens_existing_loose_owner_only_dirs() {
        use std::os::unix::fs::PermissionsExt;
        let home = TempDir::new().unwrap();
